          ...
          {cards.map(({ session, groupLabel, projectId }) => {
              ...
              >
                ...
                <div className="flex-1 min-h-0 relative">
                  ...
                  />
                  <Terminal sessionId={session.id} visible={!expanded} passiveResize hideCursor={session.task !== 'Terminal'} cliType={(session as any).cli_type as 'claude' | 'codex' | undefined} onPopOut={() => queryClient.invalidateQueries({ queryKey: ['sessions'] })} />
                </div>
              </div>
              );
            })}
